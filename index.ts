...
if (!(await exists("./output"))) {
    const cmd = new Deno.Command("git", { args: ["clone", "--depth", "1", "https://github.com/google/oss-fuzz", "./output"] });
...
for await (const dirEntry of Deno.readDir("./output/projects")) {
    const projectYAML = await Deno.readTextFile(`./output/projects/${dirEntry.name}/project.yaml`);
